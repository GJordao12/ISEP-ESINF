...
/**
 * The type Adjacency matrix graph.
 *
...
public class AdjacencyMatrixGraph<V, E> implements BasicGraph<V, E>, Cloneable {
     ...
     *
     * @param vertex the vertex
     * @return collection of vertices connected to vertex, null if vertex does not exist in the graph
     */
    public Iterable<V> directConnections(V vertex) {
        List<V> vertecies = new LinkedList<>();
        int vertexIndex = vertices.indexOf(vertex);
        for (int i = 0; i < edgeMatrix[vertexIndex].length; i++) {
            if (edgeMatrix[vertexIndex][i] != null) {
                vertecies.add(vertices.get(i));
            }
        }
        return vertecies;
    }
    ...
    public Iterable<V> adjVertices(V vert) {
        ...
        int index = toIndex(vert);
        for (int i = 0; i < edgeMatrix[index].length; i++) {
            if (edgeMatrix[index][i] != null) {
     ...
     *
     * @param vertexA vertex
     * @param vertexB vertex
     ...
     */
    public void insertVertex(V newVertex) {
    ...
    public void insertEdge(V vertexA, V vertexB, E newEdge) {
